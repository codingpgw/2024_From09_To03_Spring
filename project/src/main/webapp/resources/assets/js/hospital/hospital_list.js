document.addEventListener('DOMContentLoaded',function(){
 ...
 rows.forEach(function(row){
     row.addEventListener('dblclick',function(event){
         ...
         //userId 배열에서 추출
         const hospital_id = cells[8].innerText;  
         console.log(`hospital_id:${hospital_id}`);

         if(confirm('병원 상세 조회 하시겠습니까?') === false) return;
                     
                     //UserController.java
                     // '/user/doSelectOne.do'를 get방식으로 userId를 전달!
         window.location.href = "/ehr/hospital/doSelectOne.do?hospital_id="+hospital_id;
  ...
  doRetrieveButton.addEventListener('click',function(event){
      ...
      userForm.action = "/ehr/hospital/doRetrieve.do";    
  ...
  function moveToReg() {
      ...
      window.location.href = "/ehr/hospital/hospital_reg_index.do";
  }	
});  
